...
primOutDist <- read.csv(paste0(getwd(),"/../","excel_distributions/VFDdistributions_logodds.csv"),header=TRUE) %>%
    ...
    dplyr::select(p_hypo_pooled, p_hypo_minus10, p_hypo_05, p_hypo_10,
                  ...
                  # p_hyper_40, p_hyper_50,p_hyper_60, p_hyper_70,p_hyper_80
    ) %>%
    mutate(
        pactive_OR1_05_subgrp1 = p_hypo_05, 
        pactive_OR1_1_subgrp1 = p_hypo_10, 
        pactive_OR1_2_subgrp1 = p_hypo_20, 
        pactive_OR1_3_subgrp1 = p_hypo_30, 
        pactive_OR1_4_subgrp1 = p_hypo_40, 
...
profiles<-c("1_05","1_1","1_2","1_3","1_4","1_5")
nProfiles<-length(profiles)

Ed_simulations<-function(
                         primOutDist=primOutDist,
                         nSims = nSims,
                         interimN=interimN,
                         set.seed = m,
                         totalN = totalN,
                         ...
                         sampleCap = sampleCap,
                         efficacyOR = efficacyOR,
                         efficacyPThresh = efficacyPThresh,
                         futilityOR = futilityOR,
                         futilityPThresh = futilityPThresh,
                         mc.cores=mc.cores,
                         profiles = profiles
){
    phenoInt<-list()
    set.seed(nSims*set.seed)  
    # phenotype 1
    phenoInt[[1]]<-rep(1,nInterims)
    # phenotype 2
    phenoInt[[2]]<-c(0,rep(0:1, times=nInterims/2))
    ...
    for (m in 1:nSims) { #Repeat the study for each simulation
        ...
        for (g in 1:nPhenotypes){
            ...
            message(glue::glue('{interimN[[g]][nInterims,2]} and the probability= {primOutDist[,names(primOutDist)==paste0("pcontrol_subgrp",phenotypes[g])]}'))
            rawSimData_c[[g]]<-
        ...
        for (p in 1:nProfiles){
            for (i in 1:(nInterims)){
                ...
                #  10 categories)
                interimDataAll[[p]][[i]]$OSFD2 <- dplyr::case_when(interimDataAll[[p]][[i]]$OSFD == -1  ~ 1,
    ...
    for (g in 1:nPhenotypes){
        ...
        profileVector[[g]]<-rep(stringr::str_replace(profiles,'_','.'),nInterims)
        
        for (p in 1:nProfiles){
            ORShortVector[[g]]<-c(ORShortVector[[g]],exp(mean(postOR[[g]][[p]])))
            ORLongVector[[g]]<-c(ORLongVector[[g]],rep(exp(mean(postOR[[g]][[p]])),nInterims))
            preORLongVector[[g]]<-c(preORLongVector[[g]],rep(profileVector[[g]][p],nInterims))
        ...
        ORLongVectorAll<-c( ORLongVector[[g]],ORLongVector[[g]])
    ...
    preORLongVectorAll<-c(preORLongVector,preORLongVector)
    ...
    powerCurves<-data.frame(cbind(unlist(preORLongVector),powerVectorAll,resultVectorAll,phenoVectorAll,intNumberVectorAll,phenoIntNumberVectorAll))
...
# Ed_simulations(    primOutDist_panth=primOutDist_panth,
#                    nSims = nSims,
#                    set.seed = m,
#                    totalN = totalN,
#                    nInterims = nInterims,
...
profiles
results<-Ed_simulations(
               primOutDist=primOutDist |> dplyr::select(pactive_OR1_05_subgrp1, pcontrol_subgrp1),
               nSims = nSims,
               interimN=interimN,
               set.seed = m,
               totalN = totalN,
               ...
               futilityOR = futilityOR,
               futilityPThresh = futilityPThresh,
               profiles = "1_05") #profiles)

results$Result<-factor(results$Result,levels=c("graduated","rejected due to futility","exceeded cap","undecided"))

palette<-c("green","red","orange","cyan")

plot1<-ggplot(results[results$InterimNumber==nInterims&results$Subphenotype=="1",],
              aes(fill=Result, y=Probability, x=OR)) +
    geom_bar(position="fill", stat="identity")+
    theme(legend.position="none")+labs(x="OR in subgroup 1")+
    scale_fill_manual(values=palette)+
    geom_text(position=position_fill(vjust=0.5),aes(y=Probability,x=OR,label = ifelse(Probability==0,"",round(Probability/100,2))), size=3)
